...
import { db, auth, setFirebaseQuotaExceeded, collection, doc, onSnapshot, waitForAuthReady } from '../lib/firebase';
...
const INITIAL_SETTINGS: SystemSettings = {
  ...
  vercelTrackingUrl: 'https://laughdry-steel.vercel.app',
...
export class LaughDryDatabase {
  ...
  /**
   * [PERF] Lightweight change detection: hanya bandingkan id + updatedAt/createdAt
   * untuk menghindari JSON.stringify seluruh array yang mahal saat data besar.
   */
  private static hasListChanged<T extends { id: string; updatedAt?: string; createdAt?: string }>(
    oldList: T[],
    newList: T[]
  ): boolean {
    if (oldList.length !== newList.length) return true;
    const oldMap = new Map(oldList.map(i => [i.id, i.updatedAt || i.createdAt || '']));
    for (const item of newList) {
      const oldTs = oldMap.get(item.id);
      if (oldTs === undefined || oldTs !== (item.updatedAt || item.createdAt || '')) return true;
    }
    return false;
  }
  ...
  public static isValidOrder(o: any): boolean {
    const hasItems = Array.isArray(o.items) || Array.isArray(o.services);
    return !!(o && typeof o === 'object' && o.id && o.invoiceNumber && o.customerId && o.customerName && hasItems && typeof o.totalAmount === 'number' && o.status);
  }
  ...
  public static async syncFromFirestore(): Promise<void> {
    ...
    // [FIX] Gunakan domain spesifik 'laughdry-steel.vercel.app' bukan generic 'vercel.app'
    // agar app admin/karyawan di subdomain Vercel lain tidak salah terdeteksi sebagai customer portal
    const TRACKING_HOSTNAME = 'laughdry-steel.vercel.app';
    const currentHostname = isClient ? window.location.hostname : '';
    // [FIX] isCustomerOnlyMode hanya true jika ada sinyal eksplisit dari URL
    // (owner param + phone/invoice, atau hostname tracking). Jangan gunakan !auth.currentUser
    // karena Firebase Auth belum restore session saat fungsi ini pertama kali dipanggil.
    const isCustomerOnlyMode = isClient && (
      currentHostname === TRACKING_HOSTNAME ||
      (ownerParamUid !== null && (queryParams?.has('phone') || queryParams?.has('invoice'))) ||
      (ownerParamUid !== null && !auth.currentUser)
    );
    ...
    try {
      ...
      if (isCustomerOnlyMode) {
        ...
        /**
         * [FIX] Helper: sync customer dengan 3 strategi fallback agar UI SELALU tampil
         * meski ada masalah Firestore Security Rules untuk unauthenticated access:
         * 1. getCustomerById  — by Firestore doc ID (paling cepat)
         * 2. getCustomerByPhone — query by phone field (fallback jika rules blokir by ID)
         * 3. Synthetic customer dari data order (jika Firestore sama sekali tidak bisa diakses)
         */
        const syncCustomerWithFallback = async (order: any, phone?: string): Promise<void> => {
          let customer: any = null;

          // Strategi 1: fetch by document ID
          if (order.customerId) {
            try {
              customer = await LaundryService.getCustomerById(order.customerId);
            } catch (e) {
              console.warn('[Tracking] getCustomerById gagal, coba by phone:', e);
            }
          }

          // Strategi 2: fetch by phone number
          if (!customer) {
            const targetPhone = phone || order.customerPhone;
            if (targetPhone) {
              try {
                customer = await LaundryService.getCustomerByPhone(targetPhone);
              } catch (e) {
                console.warn('[Tracking] getCustomerByPhone gagal, pakai synthetic:', e);
              }
            }
          }

          // Strategi 3: buat synthetic customer dari data order
          // Pelanggan tetap bisa lihat status cucian meski poin/deposit tidak tersedia
          if (!customer && order.customerName) {
            const now = new Date().toISOString();
            customer = {
              id: order.customerId || `cust-${(order.customerPhone || phone || '').replace(/\D/g, '')}`,
            ...
            console.log('[Tracking] Menggunakan synthetic customer dari data order');
          }
        ...
        if (invoiceNum) {
          ...
          try {
            ...
            if (trackingOrder) {
              ...
              await syncCustomerWithFallback(trackingOrder, phoneNum || undefined);
            } else if (phoneNum) {
              // Invoice tidak ditemukan tapi phone ada — fallback ke mode phone
              console.warn('[Tracking] Invoice tidak ditemukan, fallback ke pencarian by phone');
              ...
              if (phoneOrders.length > 0) {
                ...
                await syncCustomerWithFallback(phoneOrders[0], phoneNum);
              }
            }
          } catch (e) {
          ...
          // Mode "Lacak Sekarang": cari semua order berdasarkan nomor HP pelanggan
          try {
            const phoneOrders = await LaundryService.getOrdersByPhone(phoneNum);
            if (phoneOrders.length > 0) {
              const existingIds = new Set(phoneOrders.map(o => o.id));
              ...
              // Tidak ada order → coba fetch customer langsung by phone
              try {
                const customer = await LaundryService.getCustomerByPhone(phoneNum);
                if (customer) {
                  const localCusts = this.getCustomers().filter(c => c.id !== customer.id);
                  this.saveKey('customers', [customer, ...localCusts]);
                }
              } catch (e) {
                console.warn('[Tracking] getCustomerByPhone juga gagal:', e);
              }
            }
          } catch (e) {
        ...
        // [FIX] Mulai realtime listeners setelah sync awal selesai
        // agar pelanggan mendapat update status cucian secara live tanpa perlu polling.
  ...
  public static async queueSync(
    ...
    // [FIX RACE CONDITION] Tunggu Firebase Auth selesai restore session sebelum cek uid.
    // Tanpa ini, auth.currentUser masih null saat page baru load meskipun user sudah login,
    // menyebabkan semua write di-skip dan data tidak tersimpan ke Firestore.
  ...
  private static _dispatchDebounceTimer: ReturnType<typeof setTimeout> | null = null;

  /**
   * [PERF] Debounce event dispatch: gabungkan banyak event yang terjadi bersamaan
   * dalam 1 siklus menjadi 1 dispatch, mengurangi re-render React berlebihan.
   */
  private static debouncedDispatch(eventName: string, delay = 80): void {
  ...
  public static startRealtimeListeners(): void {
    ...
    // [FIX] Baca ?owner= dari URL untuk mode tracking pelanggan (tidak login)
    const ownerParam = typeof window !== 'undefined'
      ? new URLSearchParams(window.location.search).get('owner')?.trim() || null
      : null;
    const uid = auth.currentUser?.uid || localStorage.getItem('laughdry_firebase_uid') || ownerParam;
    if (!uid) {
      console.warn("Unauthenticated. Skipping realtime listeners.");
      return;
    }
    // Pastikan localStorage juga terisi agar isOfflineOrQuota() lolos
    if (ownerParam && !localStorage.getItem('laughdry_firebase_uid')) {
    ...
    try {
      ...
      const unsubOrders = this.safeOnSnapshot(collection(db, parent, 'orders'), (snapshot) => {
        ...
        if (this.hasListChanged(currentLocal, mergedList)) {
      ...
      const unsubCustomers = this.safeOnSnapshot(collection(db, parent, 'customers'), (snapshot) => {
        ...
        if (this.hasListChanged(currentLocal, cleanList)) {
